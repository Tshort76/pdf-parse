(ns pdf-parse.tokens.pdf-extractor
  ...
  (:require [pdf-parse.tokens.char-normalize :as char-norm]
  ...
  (let [line-cutoff 10
    ...
    (proxy [PDFGraphicsStreamEngine TextStripperData] [page]
      (appendRectangle [p0 p1 p2 p3]
        (let [height (Math/abs (- (.getY p0) (.getY p2)))]
          (when (and (> (Point2D/distance (.getX p0) (.getY p0) (.getX p2) (.getY p2)) line-cutoff)
                     (or (< height 2) (> height 20)))
            (swap! position-data #(conj % (assoc (standardize {:x0 (.getX p0) :x1 (.getX p2)} top-y (.getY p0) (.getY p1) (.getY p2))
                                            :type (if (< height 2) :line :rectangle)))))))
      (clip [winding-rule])
      (closePath [])
      ...
      (lineTo [x y]
        (when (and (> (Point2D/distance (first @current-position) (second @current-position) x y) line-cutoff)
                   (zero? (.toRGB (.getNonStrokingColor (proxy-super getGraphicsState)))))
          (swap! position-data #(conj % {:x0 (first @current-position) :y0 (- top-y (second @current-position)) :x1 x :y1 (- top-y y) :type :line})))
...
(defn extract-many
  "Takes an input steam and a set of named extractors (e.g. {:text build-position-data})
  and returns the extracted values associated with their respective "
  [input-stream pages extractors]
  (with-open [doc (-> input-stream io/input-stream RandomAccessBufferedFileInputStream. PDFParser. (doto (.parse)) .getPDDocument)]
    (let [preprocessed (cons (preprocess-pdf doc) pages)]
      (->> extractors
           (map (fn [[name fnc]]
                  [name
                   (try (apply fnc preprocessed)
                        (catch Exception ex (println "Exception during pdf processing: " (.getMessage ex))))]))
           (into {})))))

(defn extract-core-properties [input-stream & pages]
  (extract-many input-stream pages {:text build-position-data
                                    :graphics build-line-position-data
                                    :page-meta page-characteristics}))
